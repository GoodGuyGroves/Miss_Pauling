...
class UserRepository:
    ...
    def get_user_by_auth_id(db: Session, provider: AuthProvider, auth_id: str) -> Optional[models.User]:
        ...
        if provider == "steam":
            return db.query(models.User).filter(models.User.steam_id64 == auth_id).first()
        elif provider == "discord":
            return db.query(models.User).filter(models.User.discord_id == auth_id).first()
        return None
