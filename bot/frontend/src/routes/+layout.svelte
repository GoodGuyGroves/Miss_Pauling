<script lang="ts">
	...
	import { user, isAuthenticated, checkAuth, login, logout } from '$lib/stores/auth';
	import { browser } from '$app/environment';

	let loading = true;

	onMount(async () => {
		// Check if the user is already authenticated with a saved token
		await checkAuth();
		loading = false;
	});
...
<div class="min-h-screen bg-gray-100">
	...
	<header class="bg-white shadow-sm">
		<div class="container mx-auto flex items-center justify-between px-4 py-4">
			<button on:click={goToHome} class="text-xl font-bold text-blue-600 hover:text-blue-700"
				>Miss Pauling</button
			>
				...
				{:else if $isAuthenticated}
					<div class="flex items-center gap-4">
						<a href="/profile" class="flex items-center transition-opacity hover:opacity-80">
							{#if $user?.avatar}
								<img src={$user.avatar} alt="User avatar" class="mr-2 h-8 w-8 rounded-full" />
							{/if}
